...
SELECT * FROM EMPLOYEE;
INSERT INTO EMPLOYEE
VALUES(4, 'QWERTY', 'QWERTY', NULL, NULL, NULL, '2015-11-11', NULL);

CREATE TRIGGER TR123
ON USERS 
AFTER INSERT AS
DECLARE @LASTNAME CHAR(20)
SELECT @LASTNAME=U_LASTNAME FROM USERS WHERE U_ID IN (SELECT MAX(U_ID) FROM USERS)
DECLARE @FIRSTNAME CHAR(20)
SELECT @FIRSTNAME=U_FIRSTNAME FROM USERS WHERE U_ID IN (SELECT MAX(U_ID) FROM USERS)
DECLARE @ID INT
SELECT @ID=MAX(EM_ID) FROM EMPLOYEE
SET @ID+=1
BEGIN
INSERT INTO EMPLOYEE(EM_ID, EM_LASTNAME, EM_FIRSTNAME)
...
INSERT INTO USERS (U_LOGIN, U_LASTNAME, U_FIRSTNAME, U_ROLE)
VALUES ('ZVER', 'PAVLOVICH', 'NIKOLAI', 1);

SELECT * FROM EMPLOYEE;
SELECT * FROM EMPLOYEE
WHERE EM_LASTNAME LIKE '%O%';

CREATE PROCEDURE TEST_COUNT AS
